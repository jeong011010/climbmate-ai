...
async def analyze_with_gpt4_vision(
    ...
    try:
        ...
        schema = (
            ...
            "  \"route\": [{\"step\": int, \"hold_id\": int, \"limb\": string (왼손/오른손/왼발/오른발), \"action\": string (한국어, 짧게), \"difficulty\": string (쉬움/중간/어려움)}] (3~7개 스텝, 아래→위 순서)\n"
            "}"
        )
        ...
        prompt = (
            ...
            f"{example}\n"
            "루트파인딩 규칙:\n"
            "1. 첫 스텝: 하단 시작 후보 중 하나(큰 y), limb='왼손' 또는 '오른손'(바닥 가까운 홀드는 '왼발'/'오른발'로 시작 가능).\n"
            "2. 마지막 스텝: 상단 탑 후보 중 하나(작은 y), limb='왼손' 또는 '오른손', action에 '탑아웃' 또는 '완등' 포함.\n"
            "3. 중간 스텝: y값 점진적 감소. 손 홀드 후 위로 뻗으려면 발 홀드 필요(손보다 낮은 y). limb를 번갈아 사용(왼손→오른손→왼발→오른발).\n"
            "4. 손/발 구분: 바닥 가까운 큰 홀드(큰 y, 큰 area)는 발 홀드 가능성 높음. 작은 홀드나 높은 위치는 손 홀드.\n"
            "5. action 구체화: 평범('왼손 잡기', '오른발 디딤'), 특수('다이노로 슬로퍼 제압', '토훅 걸고 밸런싱', '크로스 무브', '매칭', '플래깅', '탑아웃').\n"
